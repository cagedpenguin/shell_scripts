...
#      --graphics vnc \
#      --extra-args="ks=http://192.168.1.10/ks/centos7-apache_mongo.cfg ip=192.168.1.20 netmask=255.255.255.0 gateway=192.168.1.1 dns=8.8.8.8 console=tty0 console=ttyS0,115200n8" \
...
network --device eth0 --hostname wbmongo01 --bootproto=static --ip=192.168.1.20 --netmask=255.255.255.0 --gateway=192.168.1.1 --nameserver=8.8.8.8
...
# Services
services --enabled=chronyd,ntpdate,network,sshd,httpd
...
yum -y install http://yum.opensips.org/2.2/releases/el/7/x86_64/opensips-yum-releases-2.2-3.el7.noarch.rpm
yum -y update
yum -y install openssh-clients openssh-server rsync wget httpd mod_perl mlocate net-tools bind-utils tcdump
yum -y install php56u php56u-opcache php56u-pecl-mongo mongodb mongodb-server ansible
